...
static inline struct thread* thread_initial(struct thread** local)
{
	...
	do
	{
		int i;
		int index = polling;
		for(r = 0, i = 0; i < nprocs; i++, index = (index + 1) % nprocs)
		{
			if(thread = (struct thread*)channel[index].thread)
			{
				__atomic_thread_fence(__ATOMIC_ACQUIRE);
				if(__sync_bool_compare_and_swap(&channel[index].thread, thread, thread->next))
				{
					*local = thread;
